...
import { getOrderDetails } from "../../redux/features/user/order/orderAction";
import {
  CheckCircleIcon,
  TruckIcon,
  HomeIcon,
...
export default function OrderDetailPage() {
  ...
  const steps = [
    ...
    { title: "Order Deliverd", icon: HomeIcon },
  ];
    ...
    <div key={index} className="flex items-center gap-4 border-b py-4">
      ...
      <div className="flex-1">
        ...
        <p className="text-sm font-semibold mt-1">₹{item.discountPriceAtPurchase}</p>
          ...
          {/* Shipping Address */}
          <div className="items-center border-b mt-3 py-4">
            <h3 className="font-semibold mb-2">Shipping Address</h3>
            <p className="text-sm text-gray-700 mb-3 ">
              {order.address?.fullName}, {order.address?.landmark}, {order.address?.city},{" "}
              {order.address?.state} - {order.address?.zipCode}, Mobile: {order.address?.mobileNo}
            </p>
          </div>
